...
public class MessagingServiceUtils {
    ...
    public static Notification createNotification(
        ...
        if (actions != null && actions.size() > 0) {
            int usableActionCount = actions.size();
            if (usableActionCount > 3) {
                usableActionCount = 3;
            }
    ...
    private static Action createAction(
        ...
        try {
            ...
            Action notificationAction = new Action.Builder(
                    ...
                    createPendingIntent(context, remoteMessageData)).build();

            notificationAction.getExtras().putString("actionUniqueId", uniqueId);
            notificationAction.getExtras().putString("actionType", actionType);

            if ("MEAppEvent".equals(actionType)) {
                insertMeAppEventParameters(notificationAction, action);
            }
    ...
    private static void insertMeAppEventParameters(Action notificationAction, JSONObject action) throws JSONException {
        String actionEventName = action.getString("name");
        JSONObject actionEventPayload = action.optJSONObject("payload");

        notificationAction.getExtras().putString("actionEventName", actionEventName);
        if (actionEventPayload != null) {
            notificationAction.getExtras().putString("actionPayload", actionEventPayload.toString());
        }
    }
